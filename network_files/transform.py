...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def normalize(self, image):
        ...
        # [:, None, None]: shape [3] -> [3, 1, 1]  使得mean、std和image维度相同
    ...
    def max_by_axis(self, the_list):
        ...
        maxes = the_list[0] #第一张图片的shape
        for sublist in the_list[1:]:
            # sublist：得到每一张图片的shape
            for index, item in enumerate(sublist):
                maxes[index] = max(maxes[index], item)
        # maxes：这个batch中最大的channel、height、width
        return maxes

    def batch_images(self, images, size_divisible=32):
        # type: (List[Tensor], int) -> Tensor
        """
        经过之前的normalize和resize，图像的大小并不都是一样的，各自的宽高可能都不相同
        为了加快计算，通常将所有图像调整到同样的尺寸，打包成一个tensor输入到网络
        →获取一个batch中，所有图像的最大高度和最大宽度，形成一个矩形框，如果图像不能将矩形框填满，则在空的地方填充0
        这样能将所有图像调整到同一个尺寸，并且能保存图像原有比例
        ...
        if torchvision._is_tracing():
            # 将模型转换为onnx模型——开放神经网络交换格式，可以将pytorch、tensorflow、caffe等各种格式转换为onnx，
            # 然后可以通过这个格式在各个框架中进行转换，转换为onnx这个格式后，就不再依赖于各个框架了
        ...
        # 将height向上调整到stride的整数倍（对硬件更友好，能够加速运算）
    ...
    def postprocess(self,
                    result,                # type: List[Dict[str, Tensor]] #预测结果
                    image_shapes,          # type: List[Tuple[int, int]]  #图片缩放后的尺寸
                    original_image_sizes   # type: List[Tuple[int, int]]  #图片缩放前的尺寸
                    ):
        # type: (...) -> List[Dict[str, Tensor]]
        """
        对网络的预测结果进行后处理（主要将bboxes还原到原图像尺度上）
        Args:
            result: list(dict), 网络的预测结果, len(result) == batch_size  包括bounding box信息，以及bounding box类别信息
    ...
    def forward(self,
        ...
        # 保留resize之后的尺寸，方便在后面显示的时候能够映射回去
