...
def main():
    ...
    VOC_root = r"E:\研\研究生\code\faster_rcnn\VOCtrainval_11-May-2012"  # VOCdevkit
    ...
    # train_dataset 得到了经过resize和水平翻转的图片以及每个图片的targets
    train_sampler = None
    ...
    # collate_fn=val_dataset.collate_fn这是自定义的方法，如果不进行定义的话，会默认使用torch.stack()这个方法，
    # 对于之前的分类网络，对于dataset，每一个元素就是一张图像（tensor），使用torch.stack()可以实现对图像的简单拼接，得到一个一个batch
    # 这里的data既包括图像也包括图像的信息（image，target），这个collate_fn方法将一个batch中的image、target分别打包放在一起
